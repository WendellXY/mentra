...
const READ_ONLY_COMMANDS: &[&str] = &[
    ...
    "find", "which", "whereis", "whatis", "man", "file", "stat", "du", "df", "free", "uptime",
    "uname", "hostname", "whoami", "id", "groups", "env", "printenv", "echo", "printf", "date",
    "cal", "bc", "expr", "test", "true", "false", "pwd", "tree", "diff", "cmp", "md5sum",
    "sha256sum", "sha1sum", "xxd", "od", "hexdump", "strings", "readlink", "realpath", "basename",
    "dirname", "seq", "tput", "column", "jq", "yq", "xargs", "tr", "cut", "paste", "awk", "sed",
    "rg",
];
...
const SYSTEM_ADMIN_COMMANDS: &[&str] = &[
    "sudo", "su", "chroot", "mount", "umount", "fdisk", "parted", "systemctl", "service",
    "iptables", "ufw", "sysctl", "crontab", "at", "useradd", "userdel", "usermod", "groupadd",
...
pub fn check_workspace_escape(command: &str) -> ValidationResult {
    let first = extract_first_command(command);
    let is_write_cmd =
        WRITE_COMMANDS.contains(&first.as_str()) || STATE_MODIFYING_COMMANDS.contains(&first.as_str());
