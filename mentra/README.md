...
- builtin `bash`, `background_run`, `check_background`, and `files` tools
...
`Runtime::builder()` registers Mentra's builtin tools, including `bash`, `background_run`, `check_background`, `files`, and the runtime/task/team intrinsics. Shell and background execution remain disabled by default, so coding-agent setups must opt in with a runtime policy.
